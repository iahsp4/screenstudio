...
*/
	...
	/** Diese Methode f_gt durch String-Manipulation in jtpSource
	...
	public void insertListElement(String content)
	{
		...
		String source = parent.getDocumentText();
		boolean hit = false;
		String idString;
		int counter = 0;
		do
	...
	/** Diese Methode l_scht durch Stringmanipulation in jtpSource das _bergebene Element,
	  * Alternative f_r removeElement in ExtendedHTMLDocument, mit closingTag wird angegeben
	  * ob es ein schlie_enden Tag gibt
	  */
	...
	/** Diese Methode gibt jeweils den Start- und Endoffset des Elements
	  * sowie dem entsprechenden schlie_enden Tag zur_ck
	  */
	public int[] getPositions(Element element, String source, boolean closingTag, String idString)
	{
		...
		if(closingTag)
		{
			...
			// Schleife l_uft solange, bis keine neuen StartTags mehr gefunden werden
			...
			do
			{
				...
				// hitUp enth_lt die Anzahl der neuen Start-Tags
				if(hitUp == 0)
				{
					end = true;
				}
	...
	/* Diese Methode pr_ft ob der _bergebene Tag sich in der Hierachie nach oben befindet */
	...
	/* Diese Methoden geben das erste gefundende dem _bergebenen tags entsprechende Element zur_ck */
	...
	/* Diese Methoden entfernen Attribute aus dem SimpleAttributeSet, gem__ den _bergebenen Werten, und
		geben das Ergebnis als SimpleAttributeSet zur_ck*/
	...
	/* liefert den entsprechenden HTML.Tag zum Element zur_ck */
	...
	throws BadLocationException,IOException
	{
		...
		parent.refreshOnUpdate();
		int start = jtpMain.getText().indexOf(posStrings[0]);
		int end = jtpMain.getText().indexOf(posStrings[1]);
		if(start == -1 || end == -1)
		{
			return;
		}
		String htmlString = new String();
		htmlString += jtpMain.getText().substring(0,start);
		htmlString += jtpMain.getText().substring(start + posStrings[0].length(), end);
		htmlString += jtpMain.getText().substring(end + posStrings[1].length(), jtpMain.getText().length());
